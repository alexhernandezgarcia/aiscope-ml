...
def main(argv=None):
    ...
    with h5py.File(args.output_hdf5, 'w') as hdf5_file:
        ...
        images_tr = grp_tr.create_dataset('images', shape=(
            n_images, height, width, n_channels), dtype=np.uint8)
        masks_tr = grp_tr.create_dataset('masks', shape=(
            n_images, height, width, n_channels), dtype=np.uint8)
        paths_tr = grp_tr.create_dataset('paths', shape=(n_images, 1),
        ...
        for idx in tqdm(range(n_images)):
            ...
            img_cropped, mask_cropped, _ = crop_circle(img_pil_rgba, mask_pil_rgba, draw_circles=False,
                    scale=args.scale, max_circles=1)
...
def crop_circle(img_orig, mask_orig, scale=10, margin=0.05, max_circles=3, draw_circles=False):
    """
    Arguments
    ---------
    ...
    accums, cxs, cys, radii = hough_circle_peaks(hough_res, hough_radii, 
                                                 total_num_peaks=max_circles, normalize=True)
    ...
    if draw_circles:
        for cx, cy, radius in zip(cxs, cys, radii):
            ...
            draw.arc(xy=[(x_min, y_min), (x_max, y_max)], start=0, end=360, fill=(255, 20, 20), width=20)
    ...
    img_cropped = pil_pad(img_cropped, size=(size, size), color=(0, 0, 0), centering=(cx, cy))
    mask_cropped = pil_pad(mask_cropped, size=(size, size), color=(0, 0, 0), centering=(cx, cy))
